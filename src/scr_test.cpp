...
bool mcp23017_read_register_protected(uint8_t reg, uint8_t *value){
    if (xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        for (int retry = 0; retry < 3; retry++) {
            if (mcp23017_read_register(reg, value)) { 
                xSemaphoreGive(i2c_mutex); 
                return true; 
            }
        ...
        // Si falla después de reintentos, intentar recuperación
        xSemaphoreGive(i2c_mutex);
        LOGE("I2C", "MCP23017 read failed, attempting bus recovery");
        
        // Esperar un poco antes de la recuperación
        vTaskDelay(pdMS_TO_TICKS(50));
        
        // Recrear solo el bus I2C del puerto 0 para MCP23017
        i2c_driver_delete(I2C_PORT);
        vTaskDelay(pdMS_TO_TICKS(100));
        i2c_bus_init(I2C_PORT, I2C_SDA_PIN, I2C_SCL_PIN, 100000);
        init_mcp23017();
...
static inline void safe_gptimer_stop(gptimer_handle_t timer) {
    esp_err_t err = gptimer_stop(timer);
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        LOGE("GPTIMER", "gptimer_stop failed: %d", err);
    }
    // Si es ESP_ERR_INVALID_STATE (no running), lo ignoramos silenciosamente
}
void read_buttons(){
    ...
    if (!b0_active && any_on && !shutting_down && gap_ms > MAX_GAP_TOL_MS){
        ...
        { 
            ...
            safe_gptimer_stop(phases[i].timer); // Usar función segura
        }
        apply_relays();
...
static void IRAM_ATTR zero_crossing_isr_handler(void* arg){
    ...
    if (level) {
        ...
        if (phase->enabled) {
            ...
            // VERIFICAR si el timer está corriendo antes de detenerlo
            gptimer_stop(phase->timer); // Esto puede fallar si no está running
        ...
        gpio_set_level(phase->output_pin, 0);
        phase->pulse_high = false;
        gptimer_stop(phase->timer); // Esto también puede fallar
    }
}
