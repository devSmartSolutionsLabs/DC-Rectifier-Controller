...
const int maxPotVoltage = 3250;
const int minPotVoltage = 10;
const int POT_READ_INTERVAL_MS = 50;
...
void readCurrentSafe() {
    ...
    if (millis() - lastReadTime >= POT_READ_INTERVAL_MS) {
        ...
        float measured = readAveraged(adsHigh, 0, true, 0.125, 4) - offsetCurrent; 
...
void readPotenciometerSafe() {
    ...
    if (millis() - lastReadTime >= POT_READ_INTERVAL_MS) {
        ...
        if (!potManualControl) {
            ...
            int percentage ;
            if (potVoltage < maxPotVoltage / 3) {
                percentage= map(potVoltage, 0, maxPotVoltage /3, 0, 15);
            } else {
                percentage= map(potVoltage, maxPotVoltage /3 , maxPotVoltage, 16, 100);
            }
